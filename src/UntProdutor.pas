...
begin
    ...
    begin
        Application.MessageBox(pchar('Por favor, preencha o campo Produtor!'), pchar('Aviso!'), MB_ICONEXCLAMATION);
        Abort;
    end;
    ...
    begin
        ...
        begin
            Close;
            SQL.Clear;
            SQL.Add('SELECT COUNT(*) AS QT, CODPRODUTOR ||'' - ''|| NOME AS PRODUTOR ');
            SQL.Add('  FROM PRODUTOR');
            SQL.Add(' WHERE REPLACE(REPLACE(REPLACE(CPFCNPJ,''.'',''''),''/'',''''),''-'','''') = :CPFCNPJ');
            SQL.Add(' GROUP BY CODPRODUTOR, NOME');
            Params.ParamByName('CPFCNPJ').AsString := FrmPrincipal.RemoveChars(DBEdit3.Field.Text, './-');
            Open;
        end;

        if SQLQryAUX.FieldByName('QT').AsCurrency <> 0 then
        begin
            Application.MessageBox(pchar('CPF/CNPJ já Cadastrado.' +  #13#10 +'Produtor: ' + SQLQryAUX.FieldByName('PRODUTOR').AsString +'.'), pchar('Aviso!'), MB_ICONEXCLAMATION);
            Abort;
        end;
    end;
